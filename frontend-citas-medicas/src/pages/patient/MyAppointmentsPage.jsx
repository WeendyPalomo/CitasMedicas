...
const MyAppointmentsPage = () => {
  ...
  const cancelarCita = async () => {
    ...
    try {
      await appointmentService.cancelAppointment(citaSeleccionada.id, token);
      setCitas(prev =>
        prev.map(c =>
          c.id === citaSeleccionada.id ? { ...c, estado: 'cancelado' } : c
        )
      );
          ...
          <thead className="bg-gray-100">
            <tr>
              {["Estado", "Fecha", "Hora", "Médico", "Especialidad", "Acciones"].map((h) => (
                ...
                <tbody>
            {citas.map((cita) => {
              const rowStyle = cita.estado === 'cancelado'
              ...
              return (
                <tr key={cita.id} className={`border-b hover:bg-gray-50 ${rowStyle}`}>
